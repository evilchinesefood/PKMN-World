...
static void BattleUI_ReleaseAbilityPopUp(enum BattlerId);
...
void BattleUI_CreateCursorSprite(enum BattlerId battler)
{
    ...
    if (BattleUI_LoadSpriteSheet(BUI_SPRITE_GFX_CURSOR, TAG_CURSOR) == 0xFFFF
     || BattleUI_LoadSpritePalette(BUI_SPRITE_PAL_CURSOR, TAG_CURSOR) == 0xFF)
    {
    ...
    spriteId = CreateSpriteUnchecked(&sBWBattleUI_CursorTemplate, 0, 0, 0);
    if (spriteId >= MAX_SPRITES)
    {
        FreeSpriteTilesByTag(TAG_CURSOR);
        FreeSpritePaletteByTag(TAG_CURSOR);
...
{
    if (BattleUI_GetCursorSpriteId() >= MAX_SPRITES)
    {
        return;
    }
...
u32 BattleUI_LoadSpriteSheet(enum BWBattleUISpriteGraphicsType type, u32 tag)
{
    if (IndexOfSpriteTileTag(tag) != 0xFF)
        return GetSpriteTileStartByTag(tag);
...
void BattleUI_UpdateHealthbox(u8 spriteId, struct Pokemon *mon, enum BattleHealthboxElements element)
{
    ...
    if (flag & (HEALTHBOX_FLAG_LEVEL | HEALTHBOX_FLAG_NICK))
...
void BattleUI_UpdateHealthboxHPText(u32 spriteId, s32 currHp, s32 maxHp)
{
    ...
    if (!IsOnPlayerSide(battler) || (isDoubles && !gBattleSpritesDataPtr->battlerData[battler].hpNumbersNoBars))
        return;
...
void SpriteCB_BWBattleUI_HPBar(struct Sprite *sprite)
{
    ...
    sprite->x = sBWBattleUI_HealthbarCoords[coords][GetBattlerPosition(battler)][0] + 16;
    sprite->y = sBWBattleUI_HealthbarCoords[coords][GetBattlerPosition(battler)][1] + 4;
...
void BattleUI_CreateAbilityPopUp(enum BattlerId battler, enum Ability ability)
{
    ...
    {
        TestRunner_Battle_RecordAbilityPopUp(battler, gBattleScripting.abilityPopupOverwrite ? gBattleScripting.abilityPopupOverwrite : ability);
        if (gTestRunnerHeadless)
            return;
    }

    // Task allocation returns slot zero on exhaustion; check capacity first.
    BattleUI_ReleaseAbilityPopUp(battler);
    bool32 hasTaskSlot = FALSE;
    ...
    if (BattleUI_LoadSpritePalette(BUI_SPRITE_PAL_ABILITY_POP_UP, TAG_ABILITY_POP_UP) == 0xFF)
        return;
    ...
    spriteIds[0] = CreateSpriteUnchecked(&template, xCoord + xSlide,                     yCoord, 0);
    spriteIds[1] = CreateSpriteUnchecked(&template, xCoord + xSlide + TILE_TO_PIXELS(8), yCoord, 0);
    if (spriteIds[0] == MAX_SPRITES || spriteIds[1] == MAX_SPRITES)
    {
        if (spriteIds[0] != MAX_SPRITES)
            DestroySprite(&gSprites[spriteIds[0]]);
        if (spriteIds[1] != MAX_SPRITES)
            DestroySprite(&gSprites[spriteIds[1]]);
        spriteIds[0] = spriteIds[1] = MAX_SPRITES;
        BattleUI_ReleaseAbilityPopUp(battler);
        return;
    }
...
void BattleUI_DestroyAbilityPopUp(enum BattlerId battler)
{
    ...
    if (taskId >= NUM_TASKS || !gTasks[taskId].isActive
     || gTasks[taskId].func != Task_BattleUIHandleAbilityPopUp)
        return;
...
{
    enum BWBattleUISpriteGraphicsType gfx;
    enum BWBattleUISpritePaletteType pal;
    // Match named mechanics rather than assuming the donor's enum ordering.
    switch (gBattleStruct->gimmick.usableGimmick[battler])
    {
    case GIMMICK_MEGA: gfx = BUI_SPRITE_GFX_GIMMICK_TRIGGER_MEGA; pal = BUI_SPRITE_PAL_GIMMICK_TRIGGER_MEGA; break;
    case GIMMICK_Z_MOVE: gfx = BUI_SPRITE_GFX_GIMMICK_TRIGGER_Z_MOVE; pal = BUI_SPRITE_PAL_GIMMICK_TRIGGER_Z_MOVE; break;
    case GIMMICK_ULTRA_BURST: gfx = BUI_SPRITE_GFX_GIMMICK_TRIGGER_BURST; pal = BUI_SPRITE_PAL_GIMMICK_TRIGGER_BURST; break;
    case GIMMICK_DYNAMAX: gfx = BUI_SPRITE_GFX_GIMMICK_TRIGGER_DYNAMAX; pal = BUI_SPRITE_PAL_GIMMICK_TRIGGER_DYNAMAX; break;
    case GIMMICK_TERA: gfx = BUI_SPRITE_GFX_GIMMICK_TRIGGER_TERA; pal = BUI_SPRITE_PAL_GIMMICK_TRIGGER_TERA; break;
    default: return 0xFF;
    }
    if (BattleUI_LoadSpritePalette(pal, TAG_GIMMICK_TRIGGER_PAL) == 0xFF
     || BattleUI_LoadSpriteSheet(gfx, TAG_GIMMICK_TRIGGER_TILE) == 0xFFFF)
    {
        FreeSpritePaletteByTag(TAG_GIMMICK_TRIGGER_PAL);
        FreeSpriteTilesByTag(TAG_GIMMICK_TRIGGER_TILE);
        return 0xFF;
    }
    u32 sprite = CreateSpriteUnchecked(&sBWBattleUI_GimmickTriggerTemplate, TILE_TO_PIXELS(4) + 16, TILE_TO_PIXELS(14) + 16, 0);
...
{
    if (BattleUI_LoadSpriteSheet(BUI_SPRITE_GFX_MOVE_INFO_TRIGGER, MOVE_INFO_WINDOW_TAG) == 0xFFFF
     || BattleUI_LoadSpritePalette(BUI_SPRITE_PAL_TEXTBOX_0, TAG_BW_UI_TRIGGER_PAL) == 0xFF)
    {
        FreeSpriteTilesByTag(MOVE_INFO_WINDOW_TAG);
        return MAX_SPRITES;
    }
    u32 sprite = CreateSpriteUnchecked(&sBWBattleUI_MoveInfoTriggerTemplate, 16, TILE_TO_PIXELS(14) + 16, 0);
    if (sprite == MAX_SPRITES)
    {
        FreeSpriteTilesByTag(MOVE_INFO_WINDOW_TAG);
        if (GetSpriteTileStartByTag(MOVE_INFO_WINDOW_TAG) == 0xFFFF
         && GetSpriteTileStartByTag(TAG_LAST_BALL_WINDOW) == 0xFFFF)
            FreeSpritePaletteByTag(TAG_BW_UI_TRIGGER_PAL);
    }
    return sprite;
}
...
{
    if (BattleUI_LoadSpriteSheet(BUI_SPRITE_GFX_LAST_BALL_TRIGGER, TAG_LAST_BALL_WINDOW) == 0xFFFF
     || BattleUI_LoadSpritePalette(BUI_SPRITE_PAL_TEXTBOX_0, TAG_BW_UI_TRIGGER_PAL) == 0xFF)
    {
        FreeSpriteTilesByTag(TAG_LAST_BALL_WINDOW);
        return MAX_SPRITES;
    }
    u32 sprite = CreateSpriteUnchecked(&sBWBattleUI_LastBallTriggerTemplate, 16, TILE_TO_PIXELS(14) + 32, 0);
    if (sprite == MAX_SPRITES)
    {
        FreeSpriteTilesByTag(TAG_LAST_BALL_WINDOW);
        if (GetSpriteTileStartByTag(MOVE_INFO_WINDOW_TAG) == 0xFFFF
...
static void BattleUI_ReleaseAbilityPopUp(enum BattlerId battler)
{
    u32 taskId = sBWBattleUI_Resources.abilityPopUpTaskId[battler];
    if (taskId < NUM_TASKS && gTasks[taskId].isActive
     && gTasks[taskId].func == Task_BattleUIHandleAbilityPopUp)
    {
        if (sBWBattleUI_Resources.mosaicTaskId == taskId)
        {
            SetGpuReg(REG_OFFSET_MOSAIC, (GetGpuReg(REG_OFFSET_MOSAIC) & 0xFF) | sBWBattleUI_Resources.savedObjMosaic);
            sBWBattleUI_Resources.mosaicTaskId = TASK_NONE;
        }
        u32 first = gTasks[taskId].tAPU_SpriteId1;
        u32 second = gTasks[taskId].tAPU_SpriteId2;
        if (first < MAX_SPRITES)
        {
            while (IsTextPrinterActiveOnSprite(first))
                DeactivateSingleTextPrinter(first, SPRITE_TEXT_PRINTER);
            DestroySprite(&gSprites[first]);
        }
        if (second < MAX_SPRITES)
        {
            while (IsTextPrinterActiveOnSprite(second))
                DeactivateSingleTextPrinter(second, SPRITE_TEXT_PRINTER);
            DestroySprite(&gSprites[second]);
        }
        DestroyTask(taskId);
    }
...
void BattleUI_ResetGraphics(void)
{
    if (!BW_BATTLE_UI)
        return;
    BattleUI_DestroyCursorSprite();
    ...
    for (u32 task = 0; task < NUM_TASKS; task++)
        if (gTasks[task].isActive && gTasks[task].func == Task_BattleUIBounceLastBallIcon)
            DestroyTask(task);
    for (u32 i = 0; i < MAX_SPRITES; i++)
    {
        ...
        if (!sprite->inUse)
            continue;
        if (sprite->callback == SpriteCB_MoveInfoTrigger)
            DestroyMoveInfoWinGfx(sprite);
        else if (sprite->callback == SpriteCB_LastBallTrigger)
            DestroyLastUsedBallWinGfx(sprite);
        else if (sprite->callback == SpriteCB_LastBallIcon || sprite->callback == SpriteCB_BounceLastBallIcon)
            DestroyLastUsedBallGfx(sprite);
        else if (sprite->callback == SpriteCB_GimmickTrigger)
        {
...
static void Task_BattleUIHandleAbilityPopUp(u8 taskId)
{
    ...
    case APU_STATE_FADE:
    {
        if (gPaletteFade.active)
            break;
        BlendPalette(OBJ_PLTT_ID(sprite1->oam.paletteNum), 16, ++tAPU_Timer * 2, RGB_WHITE);
        if (tAPU_Timer < 8)
            break;
    ...
    case APU_STATE_MOSAIC:
    {
        ...
        if (sBWBattleUI_Resources.mosaicTaskId != TASK_NONE)
            break;
        sBWBattleUI_Resources.mosaicTaskId = taskId;
        sBWBattleUI_Resources.savedObjMosaic = GetGpuReg(REG_OFFSET_MOSAIC) & 0xFF00;
        PlaySE(SE_M_TELEPORT);
        sprite1->oam.mosaic = TRUE;
        sprite2->oam.mosaic = TRUE;
        tAPU_Mosaic = 10;
        SetGpuReg(REG_OFFSET_MOSAIC, (GetGpuReg(REG_OFFSET_MOSAIC) & 0xFF) | (tAPU_Mosaic << 12) | (tAPU_Mosaic << 8));
...
static void BattleUI_DisplayNormalMoveBox(enum BattlerId battler, struct ChooseMoveStruct *moveInfo)
{
    ...
    for (u32 i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Clear old names/PP when a slot becomes empty or changes layout.
        FillWindowPixelBuffer(windowId, PIXEL_FILL(0));
        BattleUI_DisplayMoveBoxGraphics(battler, windowId);

        // type
        enum Type type = (moveId == MOVE_NONE) ? TYPE_MYSTERY : BattleUI_GetMoveType(battler, moveId);
        ...
        if (moveId != MOVE_NONE)
        {
            // Reserve PP and effectiveness independently before fitting the complete name.
            const u8 *name = GetMoveName(moveId);
            const u8 *effect = BattleUI_GetTypeEffectivenessSymbol(battler, moveId);
            u8 ppText[8];
            bool32 showPP = gBattleResources->bufferA[battler][2] != TRUE;
            u8 *end = ConvertIntToDecimalStringN(ppText, moveInfo->currentPP[i], STR_CONV_MODE_LEFT_ALIGN, 2);
            *end++ = CHAR_SLASH;
            ConvertIntToDecimalStringN(end, moveInfo->maxPP[i], STR_CONV_MODE_LEFT_ALIGN, 2);
            u32 ppWidth = showPP ? GetStringWidth(FONT_SMALL, ppText, 0) : 0;
            u32 ppX = 104 - ppWidth;
            u32 iconWidth = GetStringWidth(FONT_SMALL, effect, 0);
            u32 nameBudget = (showPP ? ppX - 4 : 104) - 8 - iconWidth - (iconWidth ? 2 : 0);
            u32 font = GetFontIdToFit(name, FONT_SMALL, 0, nameBudget);
            u32 nameWidth = GetStringWidth(font, name, 0);
            bool32 twoRows = nameWidth > nameBudget;
            if (twoRows)
            {
                font = GetFontIdToFit(name, FONT_SMALL, 0, 96);
                nameWidth = GetStringWidth(font, name, 0);
            }
            BattleUI_AddTextPrinter(windowId, font, 8, twoRows ? 0 : 4, BUI_TXTCLR_MOVE_BOX, name);
            BattleUI_AddTextPrinter(windowId, FONT_SMALL, twoRows ? 8 : 8 + nameWidth + 2,
                                   twoRows ? 10 : 4, BUI_TXTCLR_MOVE_BOX, effect);
            if (showPP)
            {
                union TextColor clr = sBWBattleUI_TextColors[BUI_TXTCLR_MOVE_BOX];
                clr.foreground = GetCurrentPPToMaxPPState(moveInfo->currentPP[i], moveInfo->maxPP[i]) + 1;
                AddTextPrinterParameterized6(windowId, FONT_SMALL, ppX, twoRows ? 10 : 4,
                                             0, 0, clr, TEXT_SKIP_DRAW, ppText);
            }
        }
...
static void BattleUI_UpdateHealthboxLvlText(u32 spriteId, struct Pokemon *mon)
{
    ...
    // Illusion disguises the name and gender, but never the actual level.
...
static void BattleUI_PrepareBattlerTextForAbilityPopUp(enum BattlerId battler, u32 spriteId, u32 speed)
{
    ...
    AddSpriteTextPrinterParameterized6(
            spriteId, GetFontIdToFit(gStringVar1, FONT_OUTLINED, 0, 104),
...
static void BattleUI_BufferBattlerTextForAbilityPopUp(enum BattlerId battler)
{
    ...
    u32 lastChar = totalChar ? gStringVar1[totalChar - 1] : EOS;
...
static void BattleUI_PrepareAbilityTextForAbilityPopUp(enum BattlerId battler, enum Ability ability, u32 spriteId, u32 speed)
{
    ...
    AddSpriteTextPrinterParameterized6(
            spriteId, GetFontIdToFit(gAbilitiesInfo[ability].name, FONT_OUTLINED, 0, 112 - x),
