...
#define I_KEY_ESCAPE_ROPE               GEN_7       // Pre-Gen8 behavior: Escape Rope is a regular consumable in the ITEMS pocket, not a Key Item.
...
#define I_KEY_ITEM_WHEEL                TRUE       // If TRUE, enables the ORAS-style SELECT registration wheel: up to I_MAX_REGISTERED_ITEMS key items can be registered to SELECT (one per D-Pad direction). NOTE: adds I_MAX_REGISTERED_ITEMS-1 u16s to SaveBlock1.
#define I_MAX_REGISTERED_ITEMS          4           // Number of key items that can be registered to SELECT when I_KEY_ITEM_WHEEL is TRUE. Slot 1 reuses the vanilla registeredItem; the rest live in registeredItemsExtra[].
...
#define I_EXP_SHARE_FLAG        FLAG_QOL_SHARED_EXP // Party-wide EXP when set (SHARED EXP options toggle).
...
// Vs. Seeker
#define I_VS_SEEKER_CHARGING        FLAG_SYS_VS_SEEKER_CHARGING // If this flag is assigned, the Vs Seeker functionality will be enabled. Region-gated in this hack: the seeker (and its Match Call suppression) only operates on Kanto (FRLG) maps; Hoenn Match Call keeps working with the seeker in the bag. Documentation for the Vs. Seeker can be found in docs/tutorials/vs_seeker.md.
