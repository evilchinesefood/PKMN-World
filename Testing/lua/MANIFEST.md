...
### Two reasons a sweep exits non-zero with every suite green

This is real, and it is confusing the first time. Neither is a regression.

- **`VerifyMapRenumber` is never run.** It is hard-coded into `run-all.sh`'s `UNRUNNABLE` list
  because its fixture — a crafted save with pre-deletion warp indices — is not tracked in the tree.
  The sweep prints it under `NOT RUN (in MANIFEST.md, no fixture in the tree)` and sets the failure
  flag, deliberately, so nobody quotes a clean count while a documented suite is silently absent.
- **`VerifyOwnerSave` is skipped on any machine but the owner's.** Its input is
  `pokemonworld.sav`, the owner's live battery save at the repo root, which is gitignored and
  untracked. On a fresh clone the sweep prints
  `VerifyOwnerSave  SKIPPED - missing fixture pokemonworld.sav` and fails.

So on a fresh clone the honest expectation is **19 green, 1 skipped, 1 not run, exit 1**. Read the
sentinel audit, not the exit code alone. If you want a gate that can go green, the fix is to commit
the two missing saves as fixtures — not to loosen the audit.
...
| [`VerifyOwnerSave.lua`](#verifyownersavelua) | `pokemonworld.sav` (untracked) | A real mid-playthrough save survives the v9 break. |
| [`VerifyMapRenumber.lua`](#verifymaprenumberlua) | crafted save — **not in the tree** | The v9 map-renumber migration repoints persisted warps. **Cannot run — no fixture.** |
...
- the crafted pre-deletion-warp save for `VerifyMapRenumber.lua`. Never committed. Rebuildable with
