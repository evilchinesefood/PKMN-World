...
#define B_FLAG_NO_WHITEOUT          FLAG_BNET_NO_WHITEOUT // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically! Assigned for the Battle Net Leader Sim (issue #5 P3); only set for the duration of a sim battle.
...
#define B_QUICK_MOVE_CURSOR_TO_RUN          TRUE // Superseded by the RUN SHORTCUT option (QoL #12, optionsRunShortcut) - both former call sites now read the option instead of this define.
