...
#define B_FLAG_NO_WHITEOUT          FLAG_BNET_NO_WHITEOUT // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically! Assigned for the Battle Net Leader Sim (issue #5 P3); only set for the duration of a sim battle. ⚠ This fork ALSO keys "no prize/Pay Day money" off this flag (BattleScript_LocalBattleWonReward) — a future non-sim use of the no-whiteout knob will silently strip trainer payouts unless that jump is reworked onto a sim-specific check.
