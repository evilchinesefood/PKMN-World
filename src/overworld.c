...
void LoadMapFromCameraTransition(u8 mapGroup, u8 mapNum)
{
    ...
    ApplyWeatherColorMapToPals(GetNumPalsInPrimary(gMapHeader.mapLayout), NUM_PALS_TOTAL - GetNumPalsInPrimary(gMapHeader.mapLayout)); // secondary pals; primary count is 6 (Emerald) or 7 (FRLG/Johto splits)
...
static void OverworldBasic(void)
{
    ...
    if (!gPaletteFade.active && --gTimeUpdateCounter <= 0)
    {
        ...
        // (gTimeOfDay advances on every call regardless of the updateBlend argument; TRUE here is
        // for gTimeBlend, which the snapshot diff below compares against.)
