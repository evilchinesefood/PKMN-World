...
u32 GetBattleMoveTarget(enum Move move, enum MoveTarget moveTarget)
{
    ...
    case TARGET_USER_OR_ALLY:
        // Deliberate divergence from upstream #10645, which adds this case but only assigns
        // in the dead-partner branch - leaving targetBattler at its 0 initialiser whenever
        // the partner is alive. Battler 0 is the player's left mon, so on that path an
        // opponent-side attacker would target an opponent, which is the very bug the commit
        // set out to fix. Nothing downstream re-picks a target here (this is the called-move
        // path: Metronome, Copycat, Cmd_setcalledmove), so fall back to the user - which is
        // what the default case did before that commit. The deliberate user-vs-ally choice is
        // made by SetFinalChosenTarget() for the AI and HandleInputChooseMove() for the player.
