        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gParties[B_TRAINER_PLAYER][*expMonId]))
                {
                    ...
                    // faintedSpecies, not gBattleMons[...].species: upstream's fix so a Transformed
                    // opponent yields its REAL species' EVs. Taking "ours" wholesale here
                    // reverts that silently (issue #120 hazard 1).
                    MonGainEVs(&gParties[B_TRAINER_PLAYER][*expMonId], faintedSpecies);
                }
                gBattleScripting.getexpState++;
            }
