...
#include "constants/cleared_obstacles.h" // generated; sizes struct RegionSave.clearedObstacleBits
...
struct BattleFrontier
{
    ...
    /*0xEFA*/ u8 battleNetBestStreak; // was unused_EFA (never read/written -> zero in old saves); best Battle Net Tower Streak, records board
    /*0xEFB*/ u8 unused_EFB;
...
struct SaveBlock2
{
    ...
    // Integrity reference for SaveBlock3's region banks (save format v7+). Carved from the SAME
    // 0x90 reserve as the fields above, so SaveBlock2 keeps its upstream size — appending instead
    // would trip SaveBlock2SizeUnchanged below and diverge the block from stock, which #19's
    // extract-ready-branch goal cares about. 0x94 is 2-aligned, so the u16 needs no padding.
    //
    // It lives in SaveBlock2 because SaveBlock2 IS covered by the sector checksum while SaveBlock3
    // is not: HandleWriteSector() checksums the SaveBlock1/2/storage chunk only, never the
    // saveBlock3Chunk riding in the same sector. A trustworthy reference for an untrusted block.
    /*0x94*/ u16 regionChecksum;
...
// Cut trees / smashed rocks that stay cleared across rezone (save format v7+).
//
// Was a 104-entry {mapGroup, mapNum, localId} list with NO eviction: past 104 clears, new
// obstacles silently stopped persisting and regrew on every rezone. The map data holds 297
// obstacles, so that cap was reached well before the end of one playthrough — it was not a
// completionist-only ceiling. Now one bit per obstacle against a generated index table:
// 64 bytes for 512 slots instead of 313 bytes for 104 entries.
//
// See include/constants/cleared_obstacles.h (generated by Testing/GenObstacleTable.py) for the
// table-hash guard that makes a map edit regrow obstacles ONCE instead of drifting silently.
...
struct RegionSave
{
    ...
    // Persistent cut trees + smashed rocks. RESHAPED in save format v7 (was a u8 count + a
...
struct SaveBlock1
{
    ...
    /*0x2B1C*/ u8 playerRoomDecorations[DECOR_MAX_PLAYERS_HOUSE];
    /*0x2B28*/ u8 playerRoomDecorationPositions[DECOR_MAX_PLAYERS_HOUSE];
    /*0x2B34*/ u8 decorationDesks[10];
    /*0x2B3E*/ u8 decorationChairs[10];
    /*0x2B48*/ u8 decorationPlants[10];
    /*0x2B52*/ u8 decorationOrnaments[30];
    /*0x2B70*/ u8 decorationMats[30];
    /*0x2B8E*/ u8 decorationPosters[10];
    /*0x2B98*/ u8 decorationDolls[40];
    /*0x2BC0*/ u8 decorationCushions[10];
    /*0x2BCC*/ TVShow tvShows[TV_SHOWS_COUNT];
    /*0x27CA*/ //u8 padding4[2];
    /*0x2F50*/ PokeNews pokeNews[POKE_NEWS_COUNT];
    /*0x2F90*/ enum Species outbreakPokemonSpecies;
    /*0x2F92*/ u8 outbreakLocationMapNum;
    /*0x2F93*/ u8 outbreakLocationMapGroup;
    /*0x2F94*/ u8 outbreakPokemonLevel;
    /*0x2F95*/ u8 outbreakUnused1;
    /*0x2F96*/ u16 outbreakUnused2;
    /*0x2F98*/ u16 outbreakPokemonMoves[MAX_MON_MOVES];
    /*0x2FA0*/ u8 outbreakUnused3;
    /*0x2FA1*/ u8 outbreakPokemonProbability;
    /*0x2FA2*/ u16 outbreakDaysLeft;
    /*0x2FA4*/ struct GabbyAndTyData gabbyAndTyData;
    /*0x2FB4*/ u16 easyChatProfile[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x2FC0*/ u16 easyChatBattleStart[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x2FCC*/ u16 easyChatBattleWon[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x2FD8*/ u16 easyChatBattleLost[EASY_CHAT_BATTLE_WORDS_COUNT];
    /*0x2FE4*/ struct Mail mail[MAIL_COUNT];
    /*0x3224*/ u8 unlockedTrendySayings[NUM_TRENDY_SAYING_BYTES]; // Bitfield for unlockable Easy Chat words in EC_GROUP_TRENDY_SAYING
    /*0x2E25*/ //u8 padding5[3];
    /*0x322C*/ OldMan oldMan;
    /*0x326c*/ struct DewfordTrend dewfordTrends[SAVED_TRENDS_COUNT];
    /*0x3294*/ struct ContestWinner contestWinners[NUM_CONTEST_WINNERS]; // see CONTEST_WINNER_*
    /*0x3434*/ struct DayCare daycare;
#if FREE_LINK_BATTLE_RECORDS == FALSE
    /*0x3554*/ struct LinkBattleRecords linkBattleRecords;
#endif //FREE_LINK_BATTLE_RECORDS
    /*0x35AC*/ u8 giftRibbons[NUM_GIFT_RIBBONS];
               u8 padding[4];
    /*0x35B7*/ struct ExternalEventData externalEventData;
    /*0x35CB*/ struct ExternalEventFlags externalEventFlags;
    /*0x35E0*/ struct Roamer roamer[ROAMER_COUNT];
#if FREE_ENIGMA_BERRY == FALSE
    /*0x3618*/ struct EnigmaBerry enigmaBerry;
