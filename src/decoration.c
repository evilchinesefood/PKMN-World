...
static u8 AddDecorationWindow(u8 windowIndex)
{
    ...
    DrawStdFrameWithCustomTileAndPalette(*windowId, FALSE, STD_WINDOW_BASE_TILE_NUM, 14);
...
static void ShowDecorationOnMap_(u16 mapX, u16 mapY, u8 decWidth, u8 decHeight, u16 decoration)
{
    ...
    for (j = 0; j < decHeight; j++)
    {
        ...
        {
            x = mapX + i;
            metatileBehavior = GetAttributeByMetatileIdAndMapLayout(GetNumMetatilesInPrimary(gMapHeader.mapLayout) + gDecorations[decoration].tiles[j * decWidth + i], METATILE_ATTRIBUTE_BEHAVIOR);
            layerType = GetAttributeByMetatileIdAndMapLayout(GetNumMetatilesInPrimary(gMapHeader.mapLayout) + gDecorations[decoration].tiles[j * decWidth + i], METATILE_ATTRIBUTE_LAYER_TYPE);
            ...
            if (elevation != ELEVATION_INVALID)
                MapGridSetMetatileEntryAt(x, y, (gDecorations[decoration].tiles[j * decWidth + i] + (GetNumMetatilesInPrimary(gMapHeader.mapLayout) + overlapsWall)) | impassableFlag | elevation);
            else
                MapGridSetMetatileIdAt(x, y, (gDecorations[decoration].tiles[j * decWidth + i] + (GetNumMetatilesInPrimary(gMapHeader.mapLayout) + overlapsWall)) | impassableFlag);
        }
    }
}
...
static bool8 CanPlaceDecoration(u8 taskId, const struct Decoration *decoration)
{
    ...
    case DECORPERM_PASS_FLOOR:
        for (i = 0; i < mapY; i++)
        {
            ...
            for (j = 0; j < mapX; j++)
            {
                ...
                layerType = GetAttributeByMetatileIdAndMapLayout(GetNumMetatilesInPrimary(gMapHeader.mapLayout) + decoration->tiles[(mapY - 1 - i) * mapX + j], METATILE_ATTRIBUTE_LAYER_TYPE);
    ...
    case DECORPERM_BEHIND_FLOOR:
        ...
        for (j = 0; j < mapX; j++)
        {
            curX = gTasks[taskId].tCursorX + j;
            behaviorAt = MapGridGetMetatileBehaviorAt(curX, curY);
            layerType = GetAttributeByMetatileIdAndMapLayout(GetNumMetatilesInPrimary(gMapHeader.mapLayout) + decoration->tiles[j], METATILE_ATTRIBUTE_LAYER_TYPE);
