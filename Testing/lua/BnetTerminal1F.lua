...
-- route = BFS-computed waypoints from the warp-in tile to the pedestal, against the
-- edited map.bin with NPC starting tiles as obstacles. The first attempt derived a
-- route from geometry and marched every player straight into the nurse counter.
local MAPS = {
  { tag = "HoennOldale",   g = { 0, 0, 2 }, m = { 0, 0, 2 }, grp = 2,   map = 2,  ped = { 11, 2 },
    route = { { 9, 8 }, { 9, 5 }, { 11, 5 }, { 11, 2 } } },
  { tag = "Lavaridge",      g = { 0, 0, 4 }, m = { 0, 0, 5 }, grp = 4,   map = 5,  ped = { 11, 2 },
    route = { { 9, 8 }, { 9, 5 }, { 11, 5 }, { 11, 2 } } },
  { tag = "EverGrande1F",   g = { 0, 1, 6 }, m = { 0, 1, 0 }, grp = 16,  map = 10, ped = { 7, 2 },
    route = { { 7, 3 }, { 7, 2 } } },
  { tag = "KantoViridian", g = { 0, 3, 9 }, m = { 0, 0, 4 }, grp = 39,  map = 4,  ped = { 12, 2 },
    route = { { 10, 8 }, { 10, 5 }, { 11, 5 }, { 11, 4 }, { 12, 4 }, { 12, 2 } } },
  { tag = "IndigoPlateau",  g = { 0, 4, 7 }, m = { 0, 0, 0 }, grp = 47,  map = 0,  ped = { 18, 10 },
    route = { { 17, 16 }, { 17, 12 }, { 18, 12 }, { 18, 10 } } },
...
-- Seek row 0 by pressing Up: the 3-row combined menu does not wrap, so the
-- Down-seeking pick() can never reach row 0 from a lower cursor.
local function pickTop(tag)
  for _ = 1, 12 do
    if F.menuLive() then
      for _ = 1, 4 do F.press("Up", 2); F.idle(10) end
      if F.mcur() == 0 then F.press("A", 2); F.idle(45); F.L("  pickTop " .. tag .. " ok"); return true end
      F.L("  pickTop " .. tag .. ": cursor stuck at " .. F.mcur()); return false
    end
    F.press("A", 2); F.idle(35)
  end
  F.L("  pickTop " .. tag .. ": menu never went live")
  return false
end
...
-- Clear any on-arrival scene (One Island's Bill/Celio intro locks the player)
-- and prove control before routing; without it the NEXT map's debug warp is
-- silently eaten -- R+Start does not open under a scene lock.
local function settle(tag)
  -- A advances the scene's dialogue (One Island's is Bill and Celio talking);
  -- the periodic B closes anything an A stray-opened once control is back.
  for i = 1, 60 do
    if F.ensureFree() then return true end
    F.tap("A"); F.idle(30)
    if i % 5 == 0 then F.bOut(2) end
  end
  F.shot(tag .. "_never_settled")
  return false
end

-- Walk the precomputed waypoints onto the pedestal and face the screen. The
-- pedestal is the READ tile: collision 0, elevation 3, under the screen cell.
local function toPedestal(route, tag)
  if not F.route(route, tag) then return false end
  F.face("Up")
  return true
end
...
F.run(function()
  ...
  if F.check("hub: reached the pedestal", toPedestal(HUB_ROUTE, "hub")) then
  ...
  for _, M in ipairs(MAPS) do
    ...
    if not F.warpTo(M.g[1], M.g[2], M.g[3], M.m[1], M.m[2], M.m[3], 0, 0, 0, M.grp, M.map, M.tag) then
      ...
      settle(M.tag)
      if F.check(M.tag .. ": reached the pedestal", toPedestal(M.route, M.tag)) then
        signChecks(M.tag)
      end
    end
  end
  ...
  if F.warpTo(0, 0, 2, 0, 0, 2, 0, 0, 0, 2, 2, "scaling") and settle("scaling") and toPedestal(MAPS[1].route, "scaling") then
    local bp0 = F.bp()
    F.tap("A")
    if F.check("combined menu -> SCALING row", pickTop("row_scaling"))
  ...
  if F.warpTo(1, 0, 0, 0, 0, 0, 0, 0, 0, 100, 0, "hub2") == false then
    F.check("returned to the hub", false)
  end
  if toPedestal(HUB_ROUTE, "leadersim") then
