// Standalone field save dialog used by the graphical start menu, ported from
// miriamlefae/pokeemerald-expansion feat/usm/upcoming. Duplicates the static save flow
// inside start_menu.c (kept there for the classic/Union Room menu); dedupe if the classic
// menu is ever retired. Only compiled for the graphical menu.
#include "global.h"
...
static u8 SaveConfirmSaveCallback(void)
{
    ...
    // unlandable tile, so refuse politely (mirrors start_menu.c).
    if (IsPlayerFlying()) {
