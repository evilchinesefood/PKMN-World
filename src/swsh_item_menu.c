...
static const struct BgTemplate sBgTemplates_ItemMenu[] =
{
    ...
    {
        .bg = 2,
        // 256 artwork tiles + the World partner cue; keep clear of screen blocks 28-31.
        .charBaseIndex = 2,
        .mapBaseIndex = 29,
        .screenSize = 0,
        .paletteMode = 0,
...
#if I_KEY_ITEM_WHEEL
// Tag immune to blending; doesn't conflict with the TAG_* values above
#define PAL_TAG_KEY_ITEM_WHEEL  0x9000
// Per-slot registered indicators (one per D-Pad direction); index by RegisteredItemIndex
static const u8 sRegisterUp_Gfx[]    = INCGFX_U8("graphics/bag/swsh/select_button_up.png", ".4bpp");
static const u8 sRegisterRight_Gfx[] = INCGFX_U8("graphics/bag/swsh/select_button_right.png", ".4bpp");
static const u8 sRegisterDown_Gfx[]  = INCGFX_U8("graphics/bag/swsh/select_button_down.png", ".4bpp");
static const u8 sRegisterLeft_Gfx[]  = INCGFX_U8("graphics/bag/swsh/select_button_left.png", ".4bpp");
static const u8 *const sRegisteredDir_Gfx[MAX_REGISTERED_ITEMS] = {sRegisterUp_Gfx, sRegisterRight_Gfx, sRegisterDown_Gfx, sRegisterLeft_Gfx};
// Rotating box sprite for the overworld wheel
static const u32 sKeyItemBoxGfx[] = INCGFX_U32("graphics/bag/swsh/key_item_box.png", ".4bpp");
static const u16 sKeyItemBoxPal[] = INCGFX_U16("graphics/bag/swsh/key_item_box.png", ".gbapal");
...
static const u8 sMoneyLabel_Gfx[] = INCGFX_U8("graphics/bag/swsh/money.png", ".4bpp");
static const u16 sMoneyLabel_Pal[] = INCGFX_U16("graphics/bag/swsh/money.png", ".gbapal");
static const struct SpriteSheet sMoneyLabelSheet = {sMoneyLabel_Gfx, 32 * 8 / 2, TAG_FRAME_MONEY};
static const struct SpritePalette sMoneyLabelPalette = {sMoneyLabel_Pal, TAG_FRAME_MONEY};
static const struct OamData sMoneyLabelOam = {
    .shape = SPRITE_SHAPE(32x8),
    .size = SPRITE_SIZE(32x8),
    .priority = 1,
};
static const struct SpriteTemplate sMoneyLabelTemplate = {
    .tileTag = TAG_FRAME_MONEY,
    .paletteTag = TAG_FRAME_MONEY,
    .oam = &sMoneyLabelOam,
...
#if SWSH_ITEM_MENU_ACTION_IN_BATTLE
static const u8 sMultiBattleSwapPrompt_Gfx[] = INCBIN_U8("graphics/bag/swsh/multi_battle_swap_prompt_tiles.bin");
#endif
#endif
...
static const struct WindowTemplate sDefaultBagWindows[] =
{
    ...
    [WIN_APP_JAM_INFO] = {
        ...
        .height = 4,
        .paletteNum = 1,
        .baseBlock = 683, // after World's party HP-bar window (619 + 4 * 16)
    },
    [WIN_APP_JAM_LABEL] = {
        .bg = 1,
        .tilemapLeft = 18,
...
#endif
#if I_KEY_ITEM_WHEEL
// Holds the palette of the wheel's 4th (clockwise) item so the HBlank callback can scanline-copy it into place
...
#if SWSH_ITEM_MENU_ACTION_IN_BATTLE
            LoadBgTiles(2, sMultiBattleSwapPrompt_Gfx, sizeof(sMultiBattleSwapPrompt_Gfx), 256);
#endif
            SavePromptTilemap();
            gBagMenu->graphicsLoadState++;
        }
...
#if I_KEY_ITEM_WHEEL
            s32 regSlot;
            if (itemSlot.itemId != ITEM_NONE && (regSlot = RegisteredItemIndex(itemSlot.itemId)) >= 0)
                BlitBitmapToWindow(windowId, sRegisteredDir_Gfx[regSlot], 98, y + 4, 24, 16);
#else
...
static void CreatePocketScrollArrowPair(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        s16 x = sArrowX[i];
#if SWSH_ITEM_MENU_ACTION_IN_BATTLE
        if (i == 0 && BagMenu_IsMultiFull())
            x = 124; // Leave the partner and info prompts clear.
#endif
        spriteId = CreateSprite(&sSpriteTemplate_PocketArrows, x, 16, 0);
...
#if I_KEY_ITEM_WHEEL
// Returns [1-4] based on D-Pad input, or 0 otherwise. If check, requires that slot to hold an item.
static u32 DpadInputToRegisteredItemIndex(bool32 check)
{
    ...
    if (JOY_NEW(DPAD_UP))
        ...
        i = 4;
    if (i && check && GetRegisteredItem(i - 1) == ITEM_NONE)
        i = 0;
    return i;
}

static void RefreshBagAfterRegister(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    u16 *scrollPos = &gBagPosition.scrollPosition[gBagPosition.pocket];
    u16 *cursorPos = &gBagPosition.cursorPosition[gBagPosition.pocket];
...
// Waits for a D-Pad direction, then registers gSpecialVar_ItemId to that slot.
static void Task_RegisterUsingDpad(u8 taskId)
{
    u32 i;
    if (JOY_NEW(B_BUTTON))
    ...
    if (i == 0)
        return;
    PlaySE(SE_SELECT);
    SetRegisteredItem(i - 1, gSpecialVar_ItemId);
    RefreshBagAfterRegister(taskId);
}

static void ItemMenu_Register(u8 taskId)
{
    s32 index = RegisteredItemIndex(gSpecialVar_ItemId);
    u32 count = CountRegisteredItems();

    // Already registered: deselect it.
    if (index >= 0)
    {
        SetRegisteredItem(index, ITEM_NONE);
        RefreshBagAfterRegister(taskId);
        return;
    }
    // Nothing registered yet: register to slot 0 with no prompt.
    if (count == 0)
    {
        SetRegisteredItem(0, gSpecialVar_ItemId);
        RefreshBagAfterRegister(taskId);
        return;
    }
    // Otherwise prompt for which slot (D-Pad direction).
    FillWindowPixelBuffer(WIN_DESCRIPTION, PIXEL_FILL(0));
...
#if I_KEY_ITEM_WHEEL
// Scanline-copies the 4th (clockwise) item's palette into BG palette slot 13 mid-frame,
// so it shows correctly even when the screen is blend-darkened (flash caves / weather).
...
// MAX_REGISTERED_ITEMS box sprites, then their OBJWIN twins, then the icon windows
...
static void Task_KeyItemWheel(u8 taskId)
{
    ...
    case 0:
        ...
        for (i = 0; i < MAX_REGISTERED_ITEMS; i++)
        {
            ...
            if (regItem == ITEM_NONE || !CheckBagHasItem(regItem, 1))
                continue;
            tIconWindow[i] = j = AddWindowParameterized(0, sKeyItemBoxXPos[i] / 8 - 2, sKeyItemBoxYPos[i] / 8 - 2, 4, 4, i == 3 ? 13 : 13 + i, 16 * (i + 9));
            if (j == WINDOW_NONE)
                continue;
            PutWindowTilemap(j);
            BlitItemIconToWindow(regItem, j, 4, 4, i == 3 ? sKeyItemWheelExtraPalette : NULL);
            CopyWindowToVram(j, COPYWIN_FULL);
        }
        SetHBlankCallback(HBlankCB_KeyItemWheel);
        EnableInterrupts(INTR_FLAG_HBLANK);
        PlaySE(SE_WIN_OPEN);
        // In dark caves, spawn OBJWIN sprites so the boxes show through the flash window
...
bool8 UseRegisteredKeyItemOnField(void)
{
    ...
    {
        s32 index = RegisteredItemIndex(ITEM_NONE);
        u16 regItem = (index >= 0) ? GetRegisteredItem(index) : ITEM_NONE;
        if (regItem != ITEM_NONE && CheckBagHasItem(regItem, 1) == TRUE)
        {
            gSpecialVar_ItemId = regItem;
            func = GetItemFieldFunc(regItem);
        }
        else if (index >= 0)
        {
            SetRegisteredItem(index, ITEM_NONE);
...
static void DrawFrameTilemap(const u8 *tiles, u32 left, u32 top, u32 width, u32 height)
{
    u16 *buf = (u16 *)gBagMenu->mainTilemapBuffer;
    for (u32 row = 0; row < height; row++)
        for (u32 col = 0; col < width; col++)
            buf[(top + row) * 32 + left + col] = tiles[row * width + col];
    ScheduleBgCopyTilemapToVram(2);
}
...
{
    u8 windowId;
    DrawFrameTilemap(sFrameMoney_Tilemap, 0, 0, 12, 3);
    DrawFrameTilemap(sFramePrice_Tilemap, 0, 4, 10, 5);
    if (LoadSpriteSheet(&sMoneyLabelSheet) != 0xFFFF)
    {
        if (LoadSpritePalette(&sMoneyLabelPalette) != 0xFF)
            CreateSprite(&sMoneyLabelTemplate, 16, 4, 0);
        else
            FreeSpriteTilesByTag(TAG_FRAME_MONEY);
    }
...
static void PrintMoney(u8 windowId)
{
    ...
    StringExpandPlaceholders(gStringVar4, gText_PokedollarVar1);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(0));
    BagMenu_Print(windowId, FONT_NARROW, gStringVar4, GetStringRightAlignXOffset(FONT_NARROW, gStringVar4, 64), 0, 0, 0, TEXT_SKIP_DRAW, COLORID_HOVER_NAME);
    CopyWindowToVram(windowId, COPYWIN_GFX);
}

// Keep the partner L/R cue and the pocket name clear in full multi battles.
#if SWSH_ITEM_MENU_ACTION_IN_BATTLE
#define PROMPT_LEFT (gBagPosition.location == ITEMMENULOCATION_SHOP ? 7 : (BagMenu_IsMultiFull() ? 12 : 10))
#else
...
static u16 sPromptTilemapBackup[PROMPT_WIDTH * PROMPT_HEIGHT];

static void DrawPrompt(u16 baseTile)
{
    u16 *buf = (u16 *)gBagMenu->mainTilemapBuffer;
    for (u32 row = 0; row < PROMPT_HEIGHT; row++)
        for (u32 col = 0; col < PROMPT_WIDTH; col++)
            buf[(PROMPT_TOP + row) * 32 + PROMPT_LEFT + col] = baseTile + row * PROMPT_WIDTH + col;
    ScheduleBgCopyTilemapToVram(2);
}
...
static void SavePromptTilemap(void)
{
    ...
    for (u32 row = 0; row < PROMPT_HEIGHT; row++)
        for (u32 col = 0; col < PROMPT_WIDTH; col++)
            sPromptTilemapBackup[row * PROMPT_WIDTH + col] = buf[(PROMPT_TOP + row) * 32 + PROMPT_LEFT + col];
}
...
static void HidePrompt(void)
{
    ...
    for (u32 row = 0; row < PROMPT_HEIGHT; row++)
        for (u32 col = 0; col < PROMPT_WIDTH; col++)
            buf[(PROMPT_TOP + row) * 32 + PROMPT_LEFT + col] = sPromptTilemapBackup[row * PROMPT_WIDTH + col];
    ScheduleBgCopyTilemapToVram(2);
}
...
static void UpdateMoveBattleInfo(s32 itemIndex)
{
    ...
    gSprites[sMoveTypeIconSpriteId].x = 120;
...
static void SwitchMoveInfoMode(s32 itemIndex)
{
    if (sMoveInfoMode == 0)
    {
        ...
        sMoveTypeIconSpriteId = CreateSprite(&sSpriteTemplate_MoveTypeIcon, 120, 137, 0);
        sMoveTypeIconTilesPtr = (u16 *)((u8 *)OBJ_VRAM0 + 32 * GetSpriteTileStartByTag(TAG_MOVE_TYPE_ICON));
        sCategoryIconSpriteId = CreateSprite(&sSpriteTemplate_CategoryIcon, 90, 144, 0);
...
static void MergeSort(struct BagPocket *pocket, s32 (*comparator)(enum Pocket, struct ItemSlot, struct ItemSlot))
{
    ...
    // Scan the whole pocket rather than stopping at the first empty slot: tossing an
    // entire stack out of the middle leaves a hole, and stopping there would drop every
    // item past it from the sort (and so from the displayed list). usedCapacity is the
    // index just past the last occupied slot, so any holes fall inside the sorted range
    // and the comparators (which rank ITEM_NONE last) push them out to the end.
    u32 usedCapacity = 0;
...
static void UpdateBerryInfo(s32 itemIndex)
{
...

    {
        ...
        if (berryInfo->size != 0)
        {
            ...
            *ptr = EOS;
            BagMenu_Print(WIN_BERRY_INFO, FONT_SHORT_NARROW, gStringVar4, 0, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);
        }

        if (berryInfo->firmness != BERRY_FIRMNESS_UNKNOWN)
            BagMenu_Print(WIN_BERRY_INFO, FONT_SHORT_NARROW, sBerryFirmnessStrings[berryInfo->firmness], GetStringRightAlignXOffset(FONT_SHORT_NARROW, sBerryFirmnessStrings[berryInfo->firmness], 88), 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);

        BagMenu_Print(WIN_BERRY_FLAVORS, FONT_SHORT_NARROW, sText_BerryFlavorSpicy, 0, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);
        BagMenu_Print(WIN_BERRY_FLAVORS, FONT_SHORT_NARROW, sText_BerryFlavorDry, 33, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);
        BagMenu_Print(WIN_BERRY_FLAVORS, FONT_SHORT_NARROW, sText_BerryFlavorSweet, 56, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);
        BagMenu_Print(WIN_BERRY_FLAVORS, FONT_SHORT_NARROW, sText_BerryFlavorBitter, 89, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);
        BagMenu_Print(WIN_BERRY_FLAVORS, FONT_SHORT_NARROW, sText_BerryFlavorSour, 123, 0, 0, 0, TEXT_SKIP_DRAW, COLORID_NORMAL);
...
static void SwitchBerryInfoMode(s32 itemIndex)
{
    if (sBerryInfoMode == 0)
    {
        ...
        sBerryFlavorMarkSpriteIds[FLAVOR_SPICY]  = CreateSprite(&sSpriteTemplate_BerryFlavorMark, 84, 136, 0);
        sBerryFlavorMarkSpriteIds[FLAVOR_DRY]    = CreateSprite(&sSpriteTemplate_BerryFlavorMark, 117, 136, 0);
        sBerryFlavorMarkSpriteIds[FLAVOR_SWEET]  = CreateSprite(&sSpriteTemplate_BerryFlavorMark, 140, 136, 0);
        sBerryFlavorMarkSpriteIds[FLAVOR_BITTER] = CreateSprite(&sSpriteTemplate_BerryFlavorMark, 173, 136, 0);
        sBerryFlavorMarkSpriteIds[FLAVOR_SOUR]   = CreateSprite(&sSpriteTemplate_BerryFlavorMark, 207, 136, 0);
...
static void BagMenu_DrawPartyHPBarPixels(u8 slot, u8 filledWidth)
{
    ...
    if (filledWidth > 16)     colorIdx = 6;    // green
    else if (filledWidth > 6) colorIdx = 7;    // yellow
    else                      colorIdx = 8;    // red
    ...
    if (filledWidth < 32)
        FillWindowPixelRect(WIN_PARTY_HP_BAR, PIXEL_FILL(5), filledWidth, y, 32 - filledWidth, 2);
...
static void BagMenu_DrawPartyHPBar(s8 slot)
{
...

    {
        ...
        else
        {
            ...
            {
            case HP_BAR_FULL:
            case HP_BAR_GREEN:  fillColorIdx = 6; break;
            case HP_BAR_YELLOW: fillColorIdx = 7; break;
            default:            fillColorIdx = 8; break;
            }
            ...
            if (hpFraction < 32)
                FillWindowPixelRect(WIN_PARTY_HP_BAR, PIXEL_FILL(5), hpFraction, y, 32 - hpFraction, 2);
        }
    }
