    ...
    while (*src != EOS)
    {
        ...
        if (*src == PLACEHOLDER_BEGIN)
        {
            ...
            case B_TXT_ATK_TRAINER_NAME_WITH_CLASS:
                ...
                {
                    // IS_FRLG is hard-0 in an ALL_REGIONS build, so upstream's branch always
                    // took the WALLY arm -- including during a Kanto catch tutorial. Ask the
                    // running region instead (issue #120 hazard 4).
...
void BattlePutTextOnWindow(const u8 *text, u8 windowId)
{
    ...
    {
        switch (windowId & ~B_WIN_COPYTOVRAM)
        {
